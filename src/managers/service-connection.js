...
export default class ServiceConnection {
     ...
     *
     * @param {string} channel identifier (name) of the function to run it
     ...
     */
    addRoute(channel, funcBody) {
        this.listeners[channel] = this.listeners[channel] || [];
        this.listeners[channel].push(funcBody);
        return this;
    }

    #emitRoute(channel, requestMessage) {
        try {
            let self = this
            let responce = {
                send: function (responce, params) {
                    ...
                    if (this.showLogs == true) console.log(`[ ]=----(${channel} responce)`);
                },
                error: function (error, params) {
                    ...
                    if (this.showLogs == true) console.log(`[ ]=----(${channel} error)`);
                }
            }
            if (this.showLogs == true) console.log(`[ ]<----(${channel} request)`);
            let fns = this.listeners[channel];
            if (!fns) return false;
            fns.forEach((f) => {
